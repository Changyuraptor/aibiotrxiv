...
async function ensureMemberStorageReady(){
  if(window.AIBIO&&window.AIBIO.memberStorageReady){
    try{await window.AIBIO.memberStorageReady();}catch(e){console.error('Member storage preload failed',e);}
  }
}
function memberPapers(){
  if(window.AIBIO&&window.AIBIO.memberPapers)return window.AIBIO.memberPapers();
  try{return JSON.parse(localStorage.getItem('aibio_member_papers')||'[]');}catch(e){return [];}
}
...
async function upsertMemberPaper(p){
  if(window.AIBIO&&window.AIBIO.memberUpsertPaperAndWait)return await window.AIBIO.memberUpsertPaperAndWait(p);
  if(window.AIBIO&&window.AIBIO.memberUpsertPaper)return window.AIBIO.memberUpsertPaper(p);
  const list=memberPapers();const idx=list.findIndex(x=>x.id===p.id);if(idx>=0)list[idx]=p;else list.push(p);return setMemberPapers(list);
}
function findDraft(id){
  if(window.AIBIO&&window.AIBIO.memberGetPaper)return window.AIBIO.memberGetPaper(id);
  return memberPapers().find(p=>p.id===id)||null;
}
...
async function bindSubmit(){
  ...
  await ensureMemberStorageReady();
  const member=currentMember();
  ...
  form.addEventListener('submit',async e=>{e.preventDefault();
    ...
    try{await upsertMemberPaper(submission);}catch(err){console.error(err);alert((err&&err.message)||'The draft could not be saved to D1. Please try again before leaving this page.');return;}
