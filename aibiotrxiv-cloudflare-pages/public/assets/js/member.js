...
function safeSet(key,value){try{localStorage.setItem(key,JSON.stringify(value));return true;}catch(e){console.error(e);alert('Browser storage is full. Please remove older test drafts or use smaller image files.');return false;}}
...
function currentMember(){const email=currentEmail();return members().find(m=>normEmail(m.email)===email)||null}
...
function logout(){localStorage.removeItem(CURRENT_KEY);location.href='/member/login/'}
...
async function registerViaApi(name,email,password,confirmPassword){
  try{
    ...
    const data=await res.json().catch(()=>({ok:false,error:'Registration response was not JSON.'}));
    return {ok:res.ok&&data.ok,data,status:res.status};
  }catch(err){
    return {ok:false,networkError:true,error:err?.message||'Registration API unavailable.'};
  }
}
function initRegister(){
  ...
  form.addEventListener('submit',async e=>{
    ...
    if(!name||!email||!password||!confirm){alert('Please complete display name, email, password, and password confirmation.');return;}
    if(!validPassword(password)){alert('Password must be at least 8 characters and include at least one letter and one number.');return;}
    if(password!==confirm){alert('Password confirmation does not match.');return;}
    const list=members();
    if(list.some(m=>normEmail(m.email)===email)){alert('This email is already registered. Please log in.');return;}

    const apiResult=await registerViaApi(name,email,password,confirm);
    if(apiResult.ok || apiResult.networkError){
      // Local browser copy is only a UI/session mirror. It must remain unverified until the email link is clicked.
      list.push({name,email,password,emailVerified:false,createdAt:new Date().toISOString(),verificationPending:true});
      setMembers(list);
      setCurrent(email);
      const box=document.getElementById('verificationPreview');
      if(box){
        box.hidden=false;
        box.innerHTML=`<h3>Check your email</h3><p>Your account has been created, but it is not verified yet. Please open the verification email sent to <strong>${escapeHtml(email)}</strong> and click the verification link. The account will become Email verified only after that link is opened.</p><p class="muted">If you do not receive the message, check spam or contact AIBioTrXiv. You cannot formally submit or pay for a manuscript until the email is verified.</p><div class="hero-actions"><a class="btn btn-ghost" href="/member/login/">Go to login</a><a class="btn btn-cyan" href="/member/dashboard/">Open dashboard</a></div>`;
      }
      return;
    }
    alert(apiResult.data?.error || apiResult.error || 'Registration failed. Please try again.');
  });
}
async function verifyViaApi(email,token){
  try{
    const res=await fetch('/api/member/verify',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({email,token})});
    const data=await res.json().catch(()=>({ok:false,error:'Verification response was not JSON.'}));
    return {ok:res.ok&&data.ok,data,status:res.status};
  }catch(err){
    return {ok:false,networkError:true,error:err?.message||'Verification API unavailable.'};
  }
}
function markLocalVerified(email){
  const ms=members();
  const idx=ms.findIndex(m=>normEmail(m.email)===email);
  if(idx>=0){ms[idx].emailVerified=true;ms[idx].verifiedAt=new Date().toISOString();ms[idx].verificationPending=false;setMembers(ms);setCurrent(email);return true;}
  return false;
}
async function initVerify(){
  ...
  if(!email||!token){
    box.innerHTML='<h1 class="page-title">Verification link required</h1><p class="muted">Please open the verification link from your email inbox.</p><a class="btn btn-ghost" href="/member/login/">Back to login</a>';
    return;
  }
  const api=await verifyViaApi(email,token);
  if(api.ok){
    markLocalVerified(email);
    box.innerHTML='<h1 class="page-title">Email verified</h1><p class="muted">Your email has been verified. You may now draft manuscripts and proceed to payment when ready.</p><a class="btn btn-cyan" href="/member/dashboard/">Open member dashboard</a>';
    return;
  }
  // Fallback only for old local prototype tokens. New registrations do not reveal verification links on screen.
  let list=tokens();
  const tok=list.find(t=>normEmail(t.email)===email&&t.token===token&&!t.used);
  if(tok&&markLocalVerified(email)){
    tok.used=true;setTokens(list);
    box.innerHTML='<h1 class="page-title">Email verified</h1><p class="muted">Your email has been verified. You may now draft manuscripts and proceed to payment when ready.</p><a class="btn btn-cyan" href="/member/dashboard/">Open member dashboard</a>';
    return;
  }
  box.innerHTML=`<h1 class="page-title">Verification failed</h1><p class="muted">${escapeHtml(api.data?.error || 'This verification link is invalid, expired, or already used.')}</p><a class="btn btn-ghost" href="/member/login/">Back to login</a>`;
}
function initLogin(){
  ...
  form.addEventListener('submit',e=>{
    ...
    const m=members().find(x=>normEmail(x.email)===email&&x.password===password);
    if(!m){alert('Login failed. Please check your email and password.');return;}
    setCurrent(email);
    if(!m.emailVerified){
      alert('Please verify your email first. Open the verification email sent to your inbox and click the verification link.');
...
function initDashboard(){
  ...
  const verifyBlock=m.emailVerified?'':'<div class="auth-required"><h3>Email verification required</h3><p>You can draft manuscripts, but formal submission and PayPal payment require verified email. Please open the verification email sent to your inbox and click the verification link.</p></div>';
...
function initPayment(){const el=document.getElementById('paypalPaymentBox');if(!el)return;const m=currentMember();const id=new URLSearchParams(location.search).get('id')||'';const p=getPaper(id);if(!m){el.innerHTML='<div class="auth-required"><h2>Please log in first</h2><p>You must be logged in before paying the processing fee.</p><a class="btn btn-cyan" href="/member/login/">Login</a></div>';return;} if(isSuspended(m)){el.innerHTML=`<div class="auth-required"><h2>Account suspended</h2><p>${escapeHtml(suspensionMessage(m))}</p></div>`;return;} if(!m.emailVerified){el.innerHTML='<div class="auth-required"><h2>Email verification required</h2><p>Please open the verification email sent to your inbox and click the verification link before payment and formal submission.</p></div>';return;} if(!p||normEmail(p.memberEmail)!==normEmail(m.email)){el.innerHTML='<div class="auth-required"><h2>Draft not found</h2><p>This payment page could not find a draft owned by your account.</p><a class="btn btn-ghost" href="/member/dashboard/">Back to dashboard</a></div>';return;}
...
document.addEventListener('DOMContentLoaded',()=>{updateMemberNav();initRegister();initVerify();initLogin();initDashboard();initMemberSubmissions();initMemberVersions();initPayment();});
})();
