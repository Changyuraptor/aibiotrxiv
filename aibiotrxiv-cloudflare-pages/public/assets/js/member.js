...
async function setPapersAndWait(list){return await safeSetAndWait(PAPERS_KEY,list)}
...
async function setPaidAdminSubmissionsAndWait(list){return await safeSetAndWait('aibio_submissions',list)}
...
async function copyPaidPaperToAdminQueueAndWait(p){const admin=paidAdminSubmissions().filter(x=>x.id!==p.id);admin.push({...p,status:'submitted',paymentStatus:p.paymentStatus||'paid',reviewStatus:'under_review'});await setPaidAdminSubmissionsAndWait(admin);return true}
...
function memberPrintPdfPreview(id){
  memberPersistLayoutEdits(id,{silent:true});
  document.querySelectorAll('#memberWordPdfEditor .word-figure.selected').forEach(fig=>fig.classList.remove('selected'));
  document.body.classList.add('print-member-editor-preview-only');
  setTimeout(()=>{window.print();setTimeout(()=>document.body.classList.remove('print-member-editor-preview-only'),500);},80);
}
async function attachMemberPdfToPublishedRecord(record,acceptedId){
  ...
  const res=await fetch('/api/publish-pdf',{method:'POST',headers:{'Content-Type':'application/json'},credentials:'include',body:JSON.stringify({record,articleId,version,filename,htmlFragment,sourceSubmissionId:record.sourceSubmissionId||record.id||'',acceptedId:acceptedId||record.id||''})});
  const data=await res.json().catch(()=>({}));
...
async function submitWithoutPayment(id){
  ...
  await upsertPaperAndWait(updated);await copyPaidPaperToAdminQueueAndWait(updated);await notifyAdminPayment(updated,'payment-waived-test-account');location.href='/member/dashboard/?submitted=1';
}
