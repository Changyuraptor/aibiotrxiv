...
function linkCitationHtml(html){
  ...
  const walker=document.createTreeWalker(root.content,NodeFilter.SHOW_TEXT,{acceptNode(node){
    const parent=node.parentElement;
    if(!parent) return NodeFilter.FILTER_REJECT;
    if(parent.closest('a,script,style,textarea')) return NodeFilter.FILTER_REJECT;
...
function renderPeerComments(m){
  ...
  if(form){form.addEventListener('submit',async e=>{e.preventDefault();const text=document.getElementById('peerCommentText').value.trim();if(text.length<10){alert('Please write a more substantive peer comment.');return;}if(text.length>3000){alert('Please keep peer comments under 3000 characters.');return;}const banned=/(buy now|free money|casino|viagra|http:\/\/|https:\/\/)/i;if(banned.test(text)&&!confirm('This comment contains terms or links often associated with spam. Submit anyway?'))return;try{const res=await fetch('/api/comments',{method:'POST',credentials:'include',headers:{'Content-Type':'application/json'},body:JSON.stringify({manuscriptId:mid,manuscriptPublicId:articleKey(m)||mid,versionNumber:versionNumber(m.version||'v1'),commentText:text})});const data=await res.json().catch(()=>({}));if(!res.ok||!data.ok)throw new Error(data.error||'Could not save peer comment.');AIBIO_COMMENT_CACHE[mid]={loaded:false,rows:[]};await loadD1Comments(mid);renderPeerComments(m);}catch(err){alert(String(err?.message||err));}});}
}
