...
async function reportPeerComment(m, commentId){
  ...
  let reporter=null;
  try{reporter=await loadCommentMemberSession(false);}catch(e){reporter=null;}
  const articleId=articleKey(m)||m.id||'';
  const subject='[AIBioTrXiv] Report comment: '+articleId;
  const body=`Please describe the concern about this peer comment below this line.
...
Comment author: ${comment.name||'Member'}
Comment date: ${comment.createdAt||''}
Comment text:
${comment.text||''}

Reporter member:
${reporter?(reporter.name||reporter.displayName||'')+' <'+(reporter.email||'')+'>':'Not available from browser session'}
`;
  openGmailReportCompose({subject,body});
}
