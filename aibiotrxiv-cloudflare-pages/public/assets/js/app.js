...
async function reportArticleVersion(m){
  const reason=prompt('Please describe why this article version should be reviewed. Reports are sent to aibiotrxiv@gmail.com.');
  if(!reason || reason.trim().length<5){alert('Please provide a brief reason for the report.');return;}
  const payload={manuscriptId:m.id||articleKey(m),articleKey:articleKey(m),version:m.version||'v1',title:m.title||'',author:authorNames(m),reportReason:reason.trim(),reportedAt:new Date().toISOString()};
  try{
    const res=await fetch('/api/articles/report',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify(payload)});
    const data=await res.json().catch(()=>({}));
    if(!res.ok||!data.ok) throw new Error(data.error||'Report endpoint not available.');
    alert(data.emailSent?'Article report sent to AIBioTrXiv.':'Article report recorded locally. Email delivery requires Cloudflare email provider secrets.');
  }catch(err){
    const subject=encodeURIComponent('[AIBioTrXiv] Article version report: '+(m.id||articleKey(m)||''));
    const body=encodeURIComponent(`Article: ${articleKey(m)||m.id||''}
...
async function reportPeerComment(m, commentId){
  ...
  try{
    ...
    const data=await res.json().catch(()=>({}));
    if(!res.ok||!data.ok) throw new Error(data.error||'Report endpoint not available.');
    alert(data.emailSent?'Report sent to AIBioTrXiv.':'Report recorded locally. Email delivery requires Cloudflare email provider secrets.');
  }catch(err){
...
document.addEventListener('DOMContentLoaded',()=>{renderAreas('researchAreas');renderBrowse('browseList');bindSearchForms();renderDynamicManuscript();hydrateStaticPeerComments();bindArticleAnchorOffsets();bindImageZoom();checkArticleAdminSession().then(()=>{hydrateStaticPeerComments(); const box=document.getElementById('peer-comments'); if(box){const manuscriptId=box.dataset.manuscriptId || document.querySelector('.paper-meta')?.textContent?.split('·')?.[0]?.trim() || location.pathname; renderPeerComments({id:manuscriptId});}});});

// v42: responsive mobile/tablet navigation and viewport class helpers
function initResponsiveNavigation(){
  const topbar=document.querySelector('.topbar');
  const nav=document.querySelector('.nav');
  const links=document.querySelector('.nav-links');
  if(!topbar||!nav||!links) return;
  if(!document.querySelector('.mobile-nav-toggle')){
    ...
    btn.innerHTML='<span class="hamburger" aria-hidden="true"><span></span><span></span><span></span></span>';
    nav.appendChild(btn);
    btn.addEventListener('click',()=>{
      const open=topbar.classList.toggle('mobile-menu-open');
  ...
  const update=()=>{
    ...
    document.documentElement.classList.toggle('is-mobile',w<=700);
    document.documentElement.classList.toggle('is-tablet',w>700&&w<=1100);
    document.documentElement.classList.toggle('is-desktop',w>1100);
    if(w>1100) topbar.classList.remove('mobile-menu-open');
  };
  update();
