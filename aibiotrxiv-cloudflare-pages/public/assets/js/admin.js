...
async function attachAutoPdfToPublishedRecord(record){
  try{
    ...
    const uploaded=await uploadPublishedPdfToR2(record,pdfDataUri);
    return {...record,pdf:uploaded.url,pdfR2Key:uploaded.key,pdfFileName:uploaded.filename||makePdfFilename(record),pdfGeneratedAt:new Date().toISOString(),pdfGenerationMode:'client-html2pdf-r2'};
  }catch(err){
    console.error(err);
    alert('Automatic PDF generation or R2 upload failed, so the article was not published. Error detail: '+((err&&err.message)||err)+'\\n\\nPlease check whether R2 binding AIBIO_STORAGE is configured, whether the browser blocked the PDF generator script, or whether an image in the article is blocked by CORS.');
    return null;
  }
}
