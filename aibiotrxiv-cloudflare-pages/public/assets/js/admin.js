...
async function attachAutoPdfToPublishedRecord(record){
  try{
    ...
    return {...record,pdf:pdfDataUri,pdfFileName:makePdfFilename(record),pdfGeneratedAt:new Date().toISOString(),pdfGenerationMode:'client-html2pdf'};
  }catch(err){
    console.error(err);
    alert('The article was prepared for publication, but the automatic PDF generator failed. Please check your internet connection and try Publish again. The article was not published.');
    return null;
  }
}
