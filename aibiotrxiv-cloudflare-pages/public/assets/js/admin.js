...
function buildWordArticleHtmlFromSubmission(s){
  ...
  (s.sections||[]).forEach((sec,si)=>{
    ...
    if(sec.image){
      out.push(`<figure class="word-figure" data-figure-id="fig-${si}" contenteditable="false"><img src="${sec.image}" alt="Figure"><figcaption contenteditable="true">${escapeHtml(sec.legend||'')}</figcaption></figure>`);
    }
  });
  return out.join('\n');
}
function addWordFigureControls(root,submissionId){
  root.querySelectorAll('.word-figure').forEach((fig,i)=>{
    ...
    controls.children[0].addEventListener('click',e=>{e.preventDefault();e.stopPropagation();moveWordFigure(submissionId,fig.dataset.figureId,-1);});
    controls.children[1].addEventListener('click',e=>{e.preventDefault();e.stopPropagation();moveWordFigure(submissionId,fig.dataset.figureId,1);});
