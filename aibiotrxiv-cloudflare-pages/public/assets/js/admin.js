...
function renderLayout(){
  ...
  el.innerHTML=`<div class="single-layout-editor"><section class="layout-editor-pane"><div class="panel-label no-print">Editing workspace</div><div id="blogEditorToolbar">${blogEditorToolbarHtml(s.id)}</div>${renderWordProcessorDocument(s,'layout',{prefix:'layout',editorId:'wordPdfEditor',docClass:'editor-document blog-edit-document'})}</section></div><aside class="admin-card layout-tool single-layout-tool"><h3>PDF layout tools</h3><p class="muted">Edit the manuscript in the centered workspace. Use Print / Save as PDF to open the browser PDF preview for the same printable layout.</p><div class="hero-actions" style="margin-top:18px"><button class="btn btn-cyan" onclick="persistWordLayoutEdits('${s.id}')">Save layout edits</button><button class="btn btn-ghost" onclick="persistWordLayoutEdits('${s.id}',{silent:true});printLivePdfPreview()">Print / Save as PDF</button><button class="btn btn-cyan" onclick="persistWordLayoutEdits('${s.id}',{silent:true});publishAccepted('${s.id}')">Publish</button></div></aside>`;
