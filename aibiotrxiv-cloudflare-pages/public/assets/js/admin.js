...
function writeAdminRemoteKv(key,value){
...
async function writeAdminRemoteKvAndWait(key,value){
...
function safeSetLocal(key,value){
  ...
  writeAdminRemoteKv(key,value);
  return true;
}
async function safeSetLocalAndWait(key,value){
  try{localStorage.setItem(key,JSON.stringify(value));}catch(err){console.warn('local cache write failed',err);}
  return await writeAdminRemoteKvAndWait(key,value);
}
...
async function setUserSubmissionsAndWait(list){return await safeSetLocalAndWait('aibio_submissions',list.filter(x=>x.id!=='SUB-2026-DEMO'))}
...
async function setAcceptedAndWait(list){return await safeSetLocalAndWait('aibio_accepted',list);}
...
async function removeUserSubmissionAndWait(id){if(id==='SUB-2026-DEMO')return;await setUserSubmissionsAndWait(userSubmissions().filter(s=>s.id!==id));}
...
async function markSubmission(status){
  try{
    ...
    if(status==='accepted'){
      ...
      else localStorage.setItem('aibio_demo_submission',JSON.stringify(s));
      const accepted=acceptedList().filter(x=>x.id!==s.id);
      accepted.push({...s,status:'accepted',reviewStatus:'accepted'});
      await setAcceptedAndWait(accepted);
      await syncMemberPaperFromAdminAndWait({...s,status:'accepted',reviewStatus:'accepted'});
...
function renderPreview(){
  ...
  const s=findAcceptedOrSubmission(getCurrentId());
  el.innerHTML=`<div class="hero-actions no-print" style="margin-bottom:20px"><button class="btn btn-primary" onclick="window.print()">PDF preview</button><button class="btn btn-cyan" onclick="publishAccepted('${s.id}')">Publish</button><a class="btn btn-ghost" href="/admin/accepted/layout.html?id=${encodeURIComponent(s.id)}">Back to layout</a></div>${renderPdfDocument(s)}`;
}
...
async function generatePdfDataUriForPublishedRecord(record){
  ...
  try{
    const opt={
      ...
      html2canvas:{scale:1.7,useCORS:true,allowTaint:true,backgroundColor:'#ffffff',logging:false},
      jsPDF:{unit:'pt',format:'a4',orientation:'portrait'},
      pagebreak:{mode:['css','legacy']}
    };
    const dataUri=await window.html2pdf().set(opt).from(page).outputPdf('datauristring');
    return dataUri;
  }finally{
    host.remove();
  }
...
async function uploadPublishedPdfToR2(record,pdfDataUri){
  ...
  const res=await fetch('/api/storage/r2-object',{
    ...
    credentials:'include',
    body:JSON.stringify({key,base64,mimeType,filename,ownerType:'published_pdf',ownerId:articleId})
  });
  const data=await res.json().catch(()=>({}));
...
async function attachAutoPdfToPublishedRecord(record){
  try{
    ...
    console.error(err);
    alert('Automatic PDF generation or R2 upload failed, so the article was not published. Error detail: '+((err&&err.message)||err)+'\n\nThe site now uses Cloudflare Browser Run on the backend. Check CF_ACCOUNT_ID, CF_BROWSER_TOKEN, R2 binding AIBIO_STORAGE, and D1 binding DB in Cloudflare Pages settings.');
    return null;
  }
}
...
function markMemberPaperAccepted(record){
  ...
  const next=papers.map(p=>{
    ...
    delete q.pdf;delete q.pdfR2Key;delete q.htmlR2Key;delete q.pdfFileName;delete q.pdfGeneratedAt;
    return q;
  });
  if(changed)safeSetLocal('aibio_member_papers',next);
}
...
async function deletePublished(id){
  ...
  if(confirm('Move this published manuscript to the paper trash? The published PDF/HTML files in R2 will be deleted.')){
    ...
    if(moveToTrash(stripPublishedAssets(p),'published')){setPublished(rows.filter(x=>(x.siteId||x.id)!==id));location.reload();}
  }
}
async function recoverPublished(id){
  const rows=publishedList();const p=rows.find(x=>(x.siteId||x.id)===id);if(!p)return;
  await deleteR2ObjectsForRecord(p);
  setPublished(rows.filter(x=>(x.siteId||x.id)!==id));
  const s={...stripPublishedAssets(p),status:'accepted',reviewStatus:'accepted',id:p.sourceSubmissionId||p.id};
  delete s.siteId;delete s.publishedAt;delete s.date;delete s.versionPublishedAt;
  markMemberPaperAccepted(p);
  setAccepted([...acceptedList().filter(x=>x.id!==s.id),s]);
  location.href='/admin/accepted/';
}
async function unpublish(id){
  const published=publishedList();const idx=published.findIndex(p=>(p.siteId||p.id)===id);if(idx<0)return;
  const [paper]=published.splice(idx,1);
  await deleteR2ObjectsForRecord(paper);
  const clean=stripPublishedAssets(paper);
  clean.status='unpublished';clean.unpublishedAt=new Date().toISOString().slice(0,10);
  markMemberPaperAccepted(paper);
  const unpub=unpublishedList().filter(p=>(p.siteId||p.id)!==id);
  unpub.push(clean);setUnpublished(unpub);setPublished(published);location.href='/admin/unpublished/';
}
...
function recoverUnpublished(id){
  const rows=unpublishedList();const p=rows.find(x=>(x.siteId||x.id)===id);if(!p)return;
  setUnpublished(rows.filter(x=>(x.siteId||x.id)!==id));
  const s={...p,status:'accepted',reviewStatus:'accepted',id:p.sourceSubmissionId||p.id};
  delete s.siteId;delete s.unpublishedAt;delete s.publishedAt;delete s.date;
  setAccepted([...acceptedList().filter(x=>x.id!==s.id),s]);
  markMemberPaperAccepted(p);
  location.href='/admin/accepted/';
}
...
async function renderStorageResetPanel(){
  ...
  box.innerHTML=`<div class="admin-reset-box">
    ...
    ${disabled?'<p class="muted">To enable this testing-only button, add <code>ALLOW_FULL_STORAGE_RESET=true</code> in the Cloudflare Pages environment variables for the test deployment and redeploy. Do not enable it on production.</p>':''}
...
function printLivePdfPreview(){
  ...
  document.body.classList.add('print-editor-preview-only');
  setTimeout(()=>{
    window.print();
    setTimeout(()=>document.body.classList.remove('print-editor-preview-only'),500);
  },80);
}
function renderPdfDocument(s){
...
function renderPreview(){
  const el=document.getElementById('pdfPreview');if(!el)return;
  const s=findAcceptedOrSubmission(getCurrentId());
  el.innerHTML=`<div class="hero-actions no-print" style="margin-bottom:20px"><button class="btn btn-primary" onclick="window.print()">PDF preview</button><button class="btn btn-cyan" onclick="publishAccepted('${s.id}')">Publish</button><a class="btn btn-ghost" href="/admin/accepted/layout.html?id=${encodeURIComponent(s.id)}">Back to layout</a></div>${renderWordProcessorDocument(s,'preview',{prefix:'standalonePreview',editorId:'standalonePreviewWordPdfEditor',titleId:'standalonePreviewPdfTitle',metaId:'standalonePreviewPdfMeta'})}`;
}
