...
const ADMIN_PUBLISHED_PAGE_SIZE=100;
let adminPublishedPage=1;
function getPublishedSearchQuery(){return String(document.getElementById('publishedSearch')?.value||'').trim().toLowerCase();}
function publishedSearchText(p){return [p.title,p.siteId,p.id,p.articleId,p.versionGroupId,p.version,p.topic,p.researchArea,p.submissionCategory,p.authors,authorNames(p),p.memberEmail,p.publishedAt,p.date].map(x=>String(x||'')).join(' ').toLowerCase();}
function filteredPublishedRows(){
  const q=getPublishedSearchQuery();
  return publishedList().filter(p=>(p.status||'published')==='published').filter(p=>!q||publishedSearchText(p).includes(q)).sort((a,b)=>String(b.publishedAt||b.date||'').localeCompare(String(a.publishedAt||a.date||''))||String(b.siteId||b.id||'').localeCompare(String(a.siteId||a.id||'')));
}
function renderPublishedPagination(total,page,totalPages){
  if(totalPages<=1)return '';
  const pages=[];
  const add=(n,label=String(n),disabled=false)=>pages.push(`<button class="btn btn-ghost admin-page-link" type="button" ${disabled?'disabled':''} data-page="${n}">${escapeHtml(label)}</button>`);
  add(Math.max(1,page-1),'‹ Prev',page<=1);
  const windowStart=Math.max(1,page-3);
  const windowEnd=Math.min(totalPages,page+3);
  if(windowStart>1){add(1,'1'); if(windowStart>2)pages.push('<span class="muted admin-page-ellipsis">…</span>');}
  for(let i=windowStart;i<=windowEnd;i++)pages.push(`<button class="btn ${i===page?'btn-cyan':'btn-ghost'} admin-page-link" type="button" data-page="${i}">${i}</button>`);
  if(windowEnd<totalPages){if(windowEnd<totalPages-1)pages.push('<span class="muted admin-page-ellipsis">…</span>'); add(totalPages,String(totalPages));}
  add(Math.min(totalPages,page+1),'Next ›',page>=totalPages);
  return `<div class="admin-pagination" style="display:flex;flex-wrap:wrap;gap:8px;align-items:center;justify-content:center;margin:22px 0"><span class="muted">Page ${page} of ${totalPages}</span>${pages.join('')}</div>`;
}
function renderPublished(){
  const el=document.getElementById('publishedList');if(!el)return;
  const all=publishedList().filter(p=>(p.status||'published')==='published');
  const rows=filteredPublishedRows();
  const totalPages=Math.max(1,Math.ceil(rows.length/ADMIN_PUBLISHED_PAGE_SIZE));
  if(adminPublishedPage>totalPages)adminPublishedPage=totalPages;
  if(adminPublishedPage<1)adminPublishedPage=1;
  const start=(adminPublishedPage-1)*ADMIN_PUBLISHED_PAGE_SIZE;
  const pageRows=rows.slice(start,start+ADMIN_PUBLISHED_PAGE_SIZE);
  const q=getPublishedSearchQuery();
  const summary=`<div class="admin-list-summary" style="display:flex;justify-content:space-between;gap:12px;align-items:center;margin:0 0 12px"><span class="muted">Showing ${pageRows.length?start+1:0}-${Math.min(start+pageRows.length,rows.length)} of ${rows.length} matching published papers${q?` · ${all.length} total`:''}. Page size: ${ADMIN_PUBLISHED_PAGE_SIZE}.</span></div>`;
  if(!pageRows.length){el.innerHTML=summary+`<p class="muted">${q?'No published manuscripts match this search.':'No published manuscripts yet.'}</p>`;return;}
  el.innerHTML=summary+`<table class="admin-table"><thead><tr><th>Paper</th><th>Published</th><th>Status</th><th>Action</th></tr></thead><tbody>${pageRows.map(p=>`<tr><td><strong>${escapeHtml(p.title)}</strong><br>${escapeHtml(p.siteId||p.id)} ${p.version?`<span class="status">${escapeHtml(p.version)}</span>`:''}<br><span class="muted">${escapeHtml(authorNames(p)||p.authors||'')}</span><br><a class="small-link" href="/manuscripts/?id=${encodeURIComponent(p.siteId||p.id)}">Open public HTML page →</a></td><td>${escapeHtml(p.publishedAt||p.date||'')}</td><td><span class="status">published</span></td><td><button class="btn btn-danger" onclick="unpublish('${escapeHtml(p.siteId||p.id)}')">Unpublish to accepted</button> ${tinyDelete('Delete',`deletePublished('${escapeHtml(p.siteId||p.id)}')`)} <button class="btn btn-ghost" onclick="recoverPublished('${escapeHtml(p.siteId||p.id)}')">Recover to accepted</button></td></tr>`).join('')}</tbody></table>`+renderPublishedPagination(rows.length,adminPublishedPage,totalPages);
  el.querySelectorAll('.admin-page-link').forEach(btn=>btn.addEventListener('click',()=>{adminPublishedPage=Number(btn.dataset.page)||1;renderPublished();window.scrollTo({top:0,behavior:'smooth'});}));
}
...
let adminMemberCache=null;
let adminMemberLoadError='';
function adminMembers(){return Array.isArray(adminMemberCache)?adminMemberCache:[];}
function setAdminMembers(list){adminMemberCache=Array.isArray(list)?list:[];return true;}
...
function memberAccountSuspended(m){return !!(m&&(m.suspended||m.status==='suspended'||m.accountStatus==='suspended'||m.account_status==='suspended'));}
function memberCommentRestricted(m){return !!(m&&(m.commentSuspended||m.commentStatus==='restricted'||m.comment_status==='restricted'||m.commentsRestricted));}
async function loadAdminMembers(){
  try{
    const q=(document.getElementById('memberSearch')?.value||'').trim();
    const res=await fetch('/api/admin/members'+(q?'?q='+encodeURIComponent(q):''),{credentials:'include',cache:'no-store'});
    const data=await res.json().catch(()=>({}));
    if(!res.ok||!data.ok)throw new Error(data.error||'Member list could not be loaded.');
    adminMemberCache=Array.isArray(data.members)?data.members:[];
    adminMemberLoadError='';
  }catch(err){
    console.error('Admin member list load failed',err);
    adminMemberLoadError=(err&&err.message)||String(err||'Member list load failed');
    adminMemberCache=[];
  }
  return adminMemberCache;
}
async function postAdminMemberAction(email,action,reason=''){
  const res=await fetch('/api/admin/members',{method:'POST',headers:{'Content-Type':'application/json'},credentials:'include',cache:'no-store',body:JSON.stringify({email,action,reason})});
  const data=await res.json().catch(()=>({}));
  if(!res.ok||!data.ok)throw new Error(data.error||'Member update failed.');
...
function renderMemberRow(m){
  ...
  const provider=m.authProvider||m.auth_provider||'';
  const last=m.lastLoginAt||m.last_login_at||'';
  return `<tr><td><strong>${escapeHtml(m.displayName||m.name||m.email||'')}</strong><br><span class="muted">${escapeHtml(m.email||'')}</span>${provider?`<br><span class="muted">${escapeHtml(provider)} login</span>`:''}</td><td>${accountStatus}<br>${commentStatus}<br><span class="muted">${m.emailVerified||m.email_verified?'Email verified':'Email not verified'}</span>${last?`<br><span class="muted">Last login: ${escapeHtml(last)}</span>`:''}</td><td>Total ${c.total}<br><span class="muted">Drafts ${c.drafts} · submitted ${c.submitted} · published ${c.published}</span></td><td><div class="admin-member-actions">${memberAccountSuspended(m)?`<button class="btn btn-cyan" onclick="restoreMemberAccount('${escapeHtml(m.email)}')">Restore account</button>`:`<button class="btn btn-danger" onclick="suspendMemberAccount('${escapeHtml(m.email)}')">Suspend account</button>`}${memberCommentRestricted(m)?`<button class="btn btn-cyan" onclick="restoreMemberComments('${escapeHtml(m.email)}')">Restore comments</button>`:`<button class="btn btn-ghost" onclick="restrictMemberComments('${escapeHtml(m.email)}')">Restrict comments</button>`}</div></td></tr>`;
}
async function renderMembersAdmin(){
  const box=document.getElementById('adminMembersList');if(!box)return;
  box.innerHTML='<p class="muted">Loading member records from D1...</p>';
  await loadAdminMembers();
  const rows=adminMembers();
  if(adminMemberLoadError){box.innerHTML='<p class="muted">Member records could not be loaded from D1: '+escapeHtml(adminMemberLoadError)+'</p>';return;}
  box.innerHTML=rows.length?`<p class="muted">Showing ${rows.length} member record${rows.length===1?'':'s'} from the D1 <code>members</code> table.</p><table class="admin-table"><thead><tr><th>Member</th><th>Status</th><th>Manuscripts</th><th>Moderation action</th></tr></thead><tbody>${rows.map(renderMemberRow).join('')}</tbody></table>`:'<p class="muted">No matching D1 member records. If someone has logged in with Google, confirm this admin page is connected to the same D1 database environment.</p>';
}
async function suspendMemberAccount(email){const reason=prompt('Reason for full account suspension. This blocks submissions, payments, revisions, and peer comments but does not delete the account or existing articles.','Serious policy violation');if(reason===null)return;try{await postAdminMemberAction(email,'suspend_account',reason);renderMembersAdmin();}catch(err){alert((err&&err.message)||String(err));}}
async function restoreMemberAccount(email){if(!confirm('Restore this member account?'))return;try{await postAdminMemberAction(email,'restore_account');renderMembersAdmin();}catch(err){alert((err&&err.message)||String(err));}}
async function restrictMemberComments(email){const reason=prompt('Reason for restricting peer-comment activity only. This should not affect manuscript submission or already published articles.','Improper peer comments, spam, harassment, or repeated off-topic comments');if(reason===null)return;try{await postAdminMemberAction(email,'restrict_comments',reason);renderMembersAdmin();}catch(err){alert((err&&err.message)||String(err));}}
async function restoreMemberComments(email){if(!confirm('Restore this member peer-comment privilege?'))return;try{await postAdminMemberAction(email,'restore_comments');renderMembersAdmin();}catch(err){alert((err&&err.message)||String(err));}}
...
function bindPublishedAdmin(){const input=document.getElementById('publishedSearch');if(input){let t=null;input.addEventListener('input',()=>{clearTimeout(t);t=setTimeout(()=>{adminPublishedPage=1;renderPublished();},180);});}}
function bindMembersAdmin(){const input=document.getElementById('memberSearch');if(input){let t=null;input.addEventListener('input',()=>{clearTimeout(t);t=setTimeout(()=>renderMembersAdmin(),250);});renderMembersAdmin();}}
