...
function buildWordArticleHtmlFromSubmission(s){
  ...
  (s.sections||[]).forEach((sec,si)=>{
    ...
    if(sec.image){
      out.push(`<figure class="word-figure" data-figure-id="fig-${si}" data-figure-scale="92" style="max-width:92%" contenteditable="false"><img src="${sec.image}" alt="Figure"><figcaption contenteditable="true">${escapeHtml(sec.legend||'')}</figcaption></figure>`);
    }
  });
  return out.join('\n');
}
...
function addWordFigureControls(root,submissionId){
  root.querySelectorAll('.word-figure').forEach((fig,i)=>{
    ...
    const scale=applyWordFigureScale(fig,fig.dataset.figureScale || parseInt(fig.style.maxWidth,10) || 92);
    fig.querySelector('.figure-move-controls')?.remove();
    const controls=document.createElement('div');
    controls.className='figure-move-controls figure-layout-controls no-print';
    controls.setAttribute('contenteditable','false');
    controls.innerHTML=`<div class="figure-move-row"><button type="button" title="Move figure up">▲</button><button type="button" title="Move figure down">▼</button></div><label class="figure-size-control" title="Adjust this figure size"><span>Size</span><input type="range" min="35" max="100" step="1" value="${scale}"><output>${scale}%</output></label><div class="figure-size-buttons"><button type="button" title="Smaller figure">−</button><button type="button" title="Larger figure">＋</button><button type="button" title="Reset figure size">Reset</button></div>`;
    const up=controls.querySelector('.figure-move-row button:nth-child(1)');
    const down=controls.querySelector('.figure-move-row button:nth-child(2)');
    const slider=controls.querySelector('input[type="range"]');
    const output=controls.querySelector('output');
    const small=controls.querySelector('.figure-size-buttons button:nth-child(1)');
    const large=controls.querySelector('.figure-size-buttons button:nth-child(2)');
    const reset=controls.querySelector('.figure-size-buttons button:nth-child(3)');
    up.addEventListener('click',e=>{e.preventDefault();e.stopPropagation();moveWordFigure(submissionId,fig.dataset.figureId,-1);});
    down.addEventListener('click',e=>{e.preventDefault();e.stopPropagation();moveWordFigure(submissionId,fig.dataset.figureId,1);});
    const setScale=(value,save=true)=>{
      const next=applyWordFigureScale(fig,value);
    ...
    slider.addEventListener('input',e=>{e.stopPropagation();setScale(slider.value,true);});
    small.addEventListener('click',e=>{e.preventDefault();e.stopPropagation();setScale(clampWordFigureScale(fig.dataset.figureScale)-5,true);});
    large.addEventListener('click',e=>{e.preventDefault();e.stopPropagation();setScale(clampWordFigureScale(fig.dataset.figureScale)+5,true);});
    reset.addEventListener('click',e=>{e.preventDefault();e.stopPropagation();setScale(92,true);});
...
function renderLayout(){
  ...
  el.innerHTML=`<div class="live-layout-grid"><section class="layout-editor-pane"><div class="panel-label no-print">Editing workspace</div>${renderWordProcessorDocument(s,'layout',{prefix:'layout',editorId:'wordPdfEditor',docClass:'editor-document'})}</section><section class="layout-preview-pane"><div class="panel-label no-print">Live PDF preview / Print / Save as PDF</div><div class="hero-actions no-print live-preview-actions"><button class="btn btn-primary" onclick="printLivePdfPreview()">Print / Save as PDF</button><a class="btn btn-ghost" onclick="persistWordLayoutEdits('${s.id}',{silent:true})" href="/admin/accepted/preview.html?id=${encodeURIComponent(s.id)}">Open full preview</a></div><div id="livePdfPreview" class="pdf-live-preview-shell">${renderWordProcessorDocument(s,'preview',{prefix:'live',editorId:'liveWordPdfPreviewEditor',titleId:'livePdfTitle',metaId:'livePdfMeta',docClass:'live-preview-document'})}</div></section></div><aside class="admin-card layout-tool"><h3>PDF layout tools</h3><p class="muted">This version treats the manuscript as one continuous Word-like editing surface. Text is not split by section for pagination; the browser can paginate the flowing text line by line when printing. Figures remain movable blocks: hover over a figure and use ▲ / ▼ to move it through the continuous manuscript.</p><div class="hero-actions" style="margin-top:18px"><button class="btn btn-cyan" onclick="persistWordLayoutEdits('${s.id}')">Save layout edits</button><a class="btn btn-ghost" onclick="persistWordLayoutEdits('${s.id}',{silent:true})" href="/admin/accepted/preview.html?id=${encodeURIComponent(s.id)}">Preview PDF</a><button class="btn btn-cyan" onclick="persistWordLayoutEdits('${s.id}',{silent:true});publishAccepted('${s.id}')">Publish</button></div></aside>`;
