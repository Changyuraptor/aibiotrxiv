...
function addWordFigureControls(root,submissionId){
  root.querySelectorAll('.word-figure').forEach((fig,i)=>{
    ...
    controls.innerHTML=[
      '<div class="figure-control-title">Figure layout</div>',
      '<div class="figure-move-row">',
      '<button type="button" class="fig-up" title="Move figure up">▲ Up</button>',
      '<button type="button" class="fig-down" title="Move figure down">▼ Down</button>',
      '</div>',
      '<label class="figure-size-control" title="Adjust this figure size">',
      '<span>Size</span>',
      '<input type="range" min="35" max="100" step="1" value="'+scale+'">',
      '<output>'+scale+'%</output>',
      '</label>',
      '<div class="figure-size-buttons">',
      '<button type="button" class="fig-smaller" title="Smaller figure">−</button>',
      '<button type="button" class="fig-larger" title="Larger figure">＋</button>',
      '<button type="button" class="fig-reset" title="Reset figure size">Reset</button>',
      '</div>'
    ].join('');

    const up=controls.querySelector('.fig-up');
    const down=controls.querySelector('.fig-down');
    const slider=controls.querySelector('input[type="range"]');
    const output=controls.querySelector('output');
    const small=controls.querySelector('.fig-smaller');
    const large=controls.querySelector('.fig-larger');
    const reset=controls.querySelector('.fig-reset');
