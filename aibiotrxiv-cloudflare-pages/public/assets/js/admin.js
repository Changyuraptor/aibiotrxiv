...
async function setPublishedAndWait(list){return await safeSetLocalAndWait('aibio_published',list);}
...
async function attachAutoPdfToPublishedRecord(record){
  ...
  try{
    ...
    console.warn('Server-side PDF generation failed; falling back to browser html2pdf upload.',serverErr);
    try{
      const pdfDataUri=await generatePdfDataUriForPublishedRecord(record);
      const pdfUpload=await uploadPublishedPdfToR2(record,pdfDataUri);
      const htmlUpload=await uploadPublishedHtmlSnapshotToR2(record,pdfHtml);
      return {
        ...record,
        status:'published',
        pdf:pdfUpload.url,
        pdfR2Key:pdfUpload.key,
        htmlR2Key:htmlUpload.key,
        pdfFileName:filename,
        pdfGeneratedAt:new Date().toISOString(),
        pdfGenerationMode:'browser-html2pdf-r2-fallback',
        pdfGenerationWarning:String((serverErr&&serverErr.message)||serverErr||'Server-side PDF generation failed.')
      };
    }catch(fallbackErr){
      console.error(fallbackErr);
      alert('Automatic PDF generation or R2 upload failed, so the article was not published. Server error detail: '+((serverErr&&serverErr.message)||serverErr)+'\n\nFallback error detail: '+((fallbackErr&&fallbackErr.message)||fallbackErr)+'\n\nCheck CF_ACCOUNT_ID, CF_BROWSER_TOKEN, R2 binding AIBIO_STORAGE, D1 binding DB, and whether the browser can load html2pdf.');
      return null;
    }
  }
}
