...
export async function onRequestGet(context: { request: Request; env: Env }) {
  ...
  return json({
    ...
    enabled: String(context.env.ALLOW_FULL_STORAGE_RESET || '').toLowerCase() === 'true',
...
export async function onRequestPost(context: { request: Request; env: Env }) {
  ...
  if (String(env.ALLOW_FULL_STORAGE_RESET || '').toLowerCase() !== 'true') {
    return json({
      ok: false,
      error: 'Full storage reset is disabled. Set Cloudflare Pages variable ALLOW_FULL_STORAGE_RESET=true only on the test deployment, then redeploy.'
    }, 403);
  }
