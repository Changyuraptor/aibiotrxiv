...
`;
async function exists(db:any, table:string) { return !!(await db.prepare('SELECT name FROM sqlite_schema WHERE type = 'table' AND name = ?').bind(table).first()); }
export async function onRequestGet({ env, request }: any) {
  ...
  if (secret !== env.INIT_DATABASE_SECRET) return json({ ok:false, error:'Invalid initialization secret.' }, 403);
  try {
    const result = await env.DB.exec(SCHEMA);
    const required = ['members','app_settings','email_verification_tokens','member_manuscripts','payment_records','peer_comments','app_files','security_audit_events'];
    const after:any = {};
    for (const t of required) after[t] = await exists(env.DB, t);
    const missing = Object.entries(after).filter(([,v])=>!v).map(([k])=>k);
    return json({ ok: missing.length === 0, message: missing.length === 0 ? 'AIBioTrXiv D1 schema initialized successfully.' : 'Schema ran but some tables are missing.', after, missing, result }, missing.length === 0 ? 200 : 500);
  } catch(e:any) { return json({ ok:false, error:String(e?.message||e), stack:e?.stack||null }, 500); }
}
