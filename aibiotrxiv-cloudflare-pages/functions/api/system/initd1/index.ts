// AIBioTrXiv D1 initializer v55
// Protected route. Requires INIT_DATABASE_SECRET.

export interface Env { DB: D1Database; INIT_DATABASE_SECRET?: string; }

const COMPLETE_SCHEMA = `-- AIBioTrXiv production D1 schema v55
...
async function tableExists(db: D1Database, table: string) {
  const row = await db.prepare('SELECT name FROM sqlite_schema WHERE type = 'table' AND name = ?').bind(table).first();
  return !!row;
}
...
export async function onRequestGet(context: { request: Request; env: Env }) {
  ...
  if (!env.INIT_DATABASE_SECRET) return json({ ok: false, error: 'INIT_DATABASE_SECRET is not configured.' }, 500);
  if (provided !== env.INIT_DATABASE_SECRET) return json({ ok: false, error: 'Invalid initialization secret.' }, 403);

  try {
    const execResult = await env.DB.exec(COMPLETE_SCHEMA);
    const required = ['members','email_verification_tokens','member_manuscripts','peer_comments','app_files','payment_records','security_audit_events','app_settings'];
    const tables: Record<string, boolean> = {};
    for (const t of required) tables[t] = await tableExists(env.DB, t);
    const missing = Object.entries(tables).filter(([, ok]) => !ok).map(([t]) => t);
    return json({
      ok: missing.length === 0,
      message: missing.length === 0 ? 'AIBioTrXiv D1 schema initialized successfully.' : 'Schema executed, but some required tables are missing.',
      tables,
      missing,
      execResult
    }, missing.length === 0 ? 200 : 500);
  } catch (err: any) {
    return json({ ok: false, error: String(err?.message || err), stack: err?.stack || null }, 500);
  }
}
