// AIBioTrXiv D1 initializer v53
// Endpoint: /api/system/init-d1?secret=YOUR_INIT_DATABASE_SECRET
// Required Cloudflare Pages secret: INIT_DATABASE_SECRET
//
// This protected endpoint initializes the Cloudflare D1 database with the full
// runtime-compatible schema. It avoids D1 Console partial-execution problems.
...
const COMPLETE_SCHEMA = `-- AIBioTrXiv production schema v49
...
export async function onRequestGet(context: { request: Request; env: Env }) {
  ...
  try {
    const before = {
      members: await tableExists(env.DB, "members"),
      app_settings: await tableExists(env.DB, "app_settings"),
      member_manuscripts: await tableExists(env.DB, "member_manuscripts"),
      security_audit_events: await tableExists(env.DB, "security_audit_events")
    };

    const d1ExecResult = await env.DB.exec(COMPLETE_SCHEMA);
