...
export async function onRequestGet(context: any) {
  ...
  const row: any = await env.DB.prepare(`
    SELECT m.id, m.name, m.display_name, m.email, m.email_verified, m.account_status, m.status, m.comment_status, m.comment_suspended, m.comment_suspension_reason, m.auth_provider, m.avatar_url
