...
export async function onRequestPost(context: { request: Request; env: Env }) {
  ...
  const objectRole = String(body.objectRole || body.ownerType || "uploaded_object");
  ...
  await env.AIBIO_STORAGE.put(key, bytes, {
    httpMetadata: {
      ...
      contentDisposition: `${mimeType.includes("pdf") || mimeType.includes("html") ? "inline" : "attachment"}; filename="${filename.replace(/"/g, "")}"`
    },
    customMetadata: {
  ...
  if (env.DB) {
    ...
    await env.DB.prepare(`INSERT INTO r2_object_registry
      ...
      ON CONFLICT(r2_key) DO UPDATE SET
        ...
        deleted_at = NULL`)
      .bind(key, key, String(body.ownerType || ""), String(body.ownerId || ""), objectRole, mimeType, bytes.length)
      .run();
  }
