...
export async function onRequestPost(context: { request: Request; env: Env }) {
  ...
  for (const item of writes) {
    ...
    const existing: any = await env.DB.prepare("SELECT value_json FROM app_kv_storage WHERE key = ?").bind(key).first();
    if (Array.isArray(nextValue) && nextValue.length === 0 && existing?.value_json && !item.allowEmptyOverwrite && !body.allowEmptyOverwrite) {
      try {
        const oldValue = JSON.parse(existing.value_json);
    ...
    await env.DB.prepare(`INSERT INTO app_kv_storage (key, value_json, updated_at)
      ...
      .bind(key, JSON.stringify(nextValue)).run();
    saved.push(key);
  }
  return json({ ok: true, saved, blocked, message: blocked.length ? "Some empty-array overwrites were blocked to protect production data." : "ok" });
}
