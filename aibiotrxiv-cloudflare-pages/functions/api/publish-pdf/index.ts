...
function refAnchorIdForPdf(n: string) {
  return 'ref-' + String(n || '').replace(/[^0-9]/g, '');
}
function linkReferenceCitationsForPdf(html: string) {
  let out = String(html || '');
  out = out.replace(/\[(\s*\d+(?:\s*,\s*\d+)*\s*)\]/g, (_m, inner) => {
    const nums = String(inner || '').split(',').map((x: string) => x.trim()).filter(Boolean);
    return '[' + nums.map((n: string) => `<a class="citation-ref" href="#${refAnchorIdForPdf(n)}">${escapeHtml(n)}</a>`).join(', ') + ']';
  });
  out = out.replace(/(<(?:p|div|li)[^>]*>\s*)(\d+)(\s*[\.)])/gi, (_m, pre, n, punc) => `${pre}<a id="${refAnchorIdForPdf(n)}" class="reference-target" href="#${refAnchorIdForPdf(n)}">${escapeHtml(n)}</a>${punc}`);
  out = out.replace(/(^|<br\s*\/?>|\n)(\s*)(\d+)(\s*[\.)])/gi, (_m, pre, space, n, punc) => `${pre}${space}<a id="${refAnchorIdForPdf(n)}" class="reference-target" href="#${refAnchorIdForPdf(n)}">${escapeHtml(n)}</a>${punc}`);
  return out;
}
...
function productionPdfCss(siteCss = "") {
  return `
    ...
    body.published-pdf-export .pdf-word-editor h1,
    body.published-pdf-export .pdf-word-editor h2,
    body.published-pdf-export .pdf-word-editor h3,
    body.published-pdf-export .pdf-word-editor h4,
    ...
    body.published-pdf-export .citation-ref,
    body.published-pdf-export .reference-target {
      color: #1f3a5f !important;
      font-weight: 700 !important;
...
async function buildFullHtml(args: { origin: string; title: string; htmlFragment: string; css: string }) {
  const noDanger = stripDangerousHtml(linkReferenceCitationsForPdf(args.htmlFragment));
...
function findPdfTrailerValue(src: string, key: string) {
  ...
  const m = re.exec(trailerBody);
  return m ? String(m[1] || "").trim() : "";
}

function findCatalogRootRef(src: string) {
  const re = /(\d+)\s+(\d+)\s+obj/g;
  let m: RegExpExecArray | null;
  while ((m = re.exec(src))) {
    const headerEnd = re.lastIndex;
    ...
    if (endObj < 0) break;
    const objText = src.slice(headerEnd, endObj);
    if (/\/Type\s*\/Catalog\b/.test(objText)) return `${m[1]} ${m[2]} R`;
    re.lastIndex = endObj + "endobj".length;
  }
  return "";
}

function findPdfSize(src: string) {
  const trailerSize = findPdfTrailerValue(src, "Size");
  const parsed = Number(trailerSize);
  if (Number.isFinite(parsed) && parsed > 0) return parsed;
  let maxObj = 0;
  const re = /(\d+)\s+(\d+)\s+obj/g;
  let m: RegExpExecArray | null;
  while ((m = re.exec(src))) maxObj = Math.max(maxObj, Number(m[1] || 0));
  return maxObj + 1;
}

function findPdfRoot(src: string) {
  const trailerRoot = findPdfTrailerValue(src, "Root");
  if (/^\d+\s+\d+\s+R$/.test(trailerRoot)) return trailerRoot;
  return findCatalogRootRef(src);
}
...
function removeFirstPageHeaderOverlay(pdfBuffer: ArrayBuffer) {
  try {
    ...
    const oldSize = findPdfSize(src);
    const rootRef = findPdfRoot(src);
    if (!rootRef || !Number.isFinite(oldSize) || oldSize <= 0) return pdfBuffer;

    const streamObj = oldSize;
    const newSize = oldSize + 1;
    const streamRef = `${streamObj} 0 R`;

    /* v82: Cloudflare's headerTemplate is applied to page 1 as well as later
       pages. There is no Browser Rendering option equivalent to
       "headerTemplate except first page", so we keep headerTemplate for stable
       page-2+ rendering and then append a white paint command to page 1 only.
       The rectangle covers the top ~27 mm of A4: enough to remove the
       template line, but still above the large first-page brand block. */
    const pageW = 596;
    const pageH = 842;
    const coverH = 78;
    const coverY = pageH - coverH;
    const coverStream = `q\n1 1 1 rg\n0 ${coverY} ${pageW} ${coverH} re\nf\nQ\n`;
...
function canonicalPreviewBase(articleId: string, version: string) {
  return `canonical-preview/${sanitizeSlug(articleId, "article")}/${sanitizeSlug(version, "v1")}`;
}

function isCanonicalPreviewKey(key: unknown, articleId: string, version: string, suffix: ".pdf" | ".html") {
  const value = String(key || "");
  const base = `${canonicalPreviewBase(articleId, version)}/`;
  return value.startsWith(base) && value.endsWith(suffix) && !value.includes("..") && !value.includes("//");
}

async function readR2ObjectBuffer(bucket: R2Bucket | undefined, key: string) {
...
async function readR2ObjectText(bucket: R2Bucket | undefined, key: string) {
  if (!bucket || !key) return null;
  const obj = await bucket.get(key);
  if (!obj) return null;
...
export async function onRequestPost(context: { request: Request; env: Env }) {
  ...
  try {
    ...
    let htmlForStorage = fullHtml;
    let reusedCanonicalPreview = false;

    const requestedPreviewPdfKey = String(body.canonicalPreviewPdfR2Key || record.canonicalPreviewPdfR2Key || "");
    const requestedPreviewHtmlKey = String(body.canonicalPreviewHtmlR2Key || record.canonicalPreviewHtmlR2Key || "");

    if (!previewOnly && requestedPreviewPdfKey && isCanonicalPreviewKey(requestedPreviewPdfKey, articleId, version, ".pdf")) {
      const cachedPdf = await readR2ObjectBuffer(env.AIBIO_STORAGE, requestedPreviewPdfKey);
      if (!cachedPdf) throw new Error("Canonical preview PDF was not found in R2. Generate PDF preview again before publishing.");
      pdfBuffer = cachedPdf;
      reusedCanonicalPreview = true;
      if (requestedPreviewHtmlKey && isCanonicalPreviewKey(requestedPreviewHtmlKey, articleId, version, ".html")) {
        htmlForStorage = (await readR2ObjectText(env.AIBIO_STORAGE, requestedPreviewHtmlKey)) || fullHtml;
      }
    } else {
      pdfBuffer = await callBrowserRunPdf(env, fullHtml);
    }
    ...
    if (previewOnly) {
      const headers: Record<string, string> = {
        ...
        "x-aibiotrxiv-pdf-mode": "cloudflare-browser-rendering-canonical-preview-v84"
      };

      if (body.storeCanonicalPreview === true && env.AIBIO_STORAGE) {
        const token = `${Date.now()}-${crypto.randomUUID()}`.replace(/[^A-Za-z0-9_-]+/g, "-");
        const base = canonicalPreviewBase(articleId, version);
        const previewPdfKey = `${base}/${token}.pdf`;
        const previewHtmlKey = `${base}/${token}.html`;
        const previewHash = await sha256Hex(pdfBuffer);
        const previewHtmlHash = await sha256Hex(fullHtml);
        await env.AIBIO_STORAGE.put(previewPdfKey, pdfBuffer, {
          httpMetadata: { contentType: "application/pdf", contentDisposition: `inline; filename="${filename.replace(/"/g, "")}"` },
          customMetadata: { articleId, version, objectRole: "canonical_preview_pdf", generatedAt: new Date().toISOString(), sha256: previewHash }
        });
        await env.AIBIO_STORAGE.put(previewHtmlKey, fullHtml, {
          httpMetadata: { contentType: "text/html; charset=utf-8", contentDisposition: `inline; filename="${articleId}-${version}-canonical-preview.html"` },
          customMetadata: { articleId, version, objectRole: "canonical_preview_html", generatedAt: new Date().toISOString(), sha256: previewHtmlHash }
        });
        headers["x-aibiotrxiv-preview-pdf-key"] = previewPdfKey;
        headers["x-aibiotrxiv-preview-html-key"] = previewHtmlKey;
        headers["x-aibiotrxiv-preview-pdf-sha256"] = previewHash;
        headers["x-aibiotrxiv-preview-html-sha256"] = previewHtmlHash;
      }
    ...
    const htmlHash = await sha256Hex(htmlForStorage);

    await env.AIBIO_STORAGE.put(htmlKey, htmlForStorage, {
      httpMetadata: { contentType: "text/html; charset=utf-8", contentDisposition: `inline; filename="${articleId}-${version}.html"` },
      customMetadata: { articleId, version, objectRole: "published_html", generatedAt: new Date().toISOString(), source: reusedCanonicalPreview ? "canonical_preview" : "fresh_render" }
    });
    ...
    await registerObject(env.DB, htmlKey, articleId, "published_html", "text/html; charset=utf-8", new TextEncoder().encode(htmlForStorage).length, htmlHash, version);
    ...
    const updatedRecord = {
      ...
      version,
      status: "published",
      pdf: `/api/storage/r2-object?key=${encodeURIComponent(pdfKey)}&v=${encodeURIComponent(pdfHash)}`,
      pdfR2Key: pdfKey,
      htmlR2Key: htmlKey,
      pdfFileName: filename,
      pdfGeneratedAt: now,
      pdfGenerationMode: reusedCanonicalPreview ? "canonical-preview-r2-published-v84" : "cloudflare-browser-run-r2-v84",
      canonicalPreviewPdfR2Key: reusedCanonicalPreview ? requestedPreviewPdfKey : record.canonicalPreviewPdfR2Key,
      canonicalPreviewHtmlR2Key: reusedCanonicalPreview ? requestedPreviewHtmlKey : record.canonicalPreviewHtmlR2Key,
      pdfSha256: pdfHash,
      htmlSha256: htmlHash
    };
